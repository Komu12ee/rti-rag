...
LEGAL_CHUNK_DIR = PROJECT_ROOT / "03_chunking" / "cgsic_output"
QDRANT_PATH = PROJECT_ROOT / "04_embeddings_and_kg" / "db" / "qdrant_local"
MANIFEST_FILE = (
    PROJECT_ROOT
    / "04_embeddings_and_kg"
    / ".embeddings_manifest_cgsic_important_decisions_v1.json"
)
...
def get_cgsic_chunk_files() -> List[Path]:
    """Return only per-decision CGSIC legal chunk files."""
    if not LEGAL_CHUNK_DIR.exists():
        return []
    return sorted(
        path
        for path in LEGAL_CHUNK_DIR.glob("CGSIC_IMPORTANT_*/legal_chunks.jsonl")
        if path.is_file()
    )


def find_new_chunks() -> List[Path]:
    """Return per-decision JSONL files containing unindexed records."""
    chunk_files = get_cgsic_chunk_files()
    if not chunk_files:
        logger.error(
            "No CGSIC decision chunk files found under "
            f"{LEGAL_CHUNK_DIR / 'CGSIC_IMPORTANT_*' / 'legal_chunks.jsonl'}"
        )
        return []
    ...
    return [
        ...
        if any(
            record["file"] not in indexed_files
            for record in _read_legal_jsonl(chunk_file)
        )
    ]


def _legal_jsonl_manifest_key(path: Path, record: dict) -> str:
    rel = path.relative_to(LEGAL_CHUNK_DIR)
    chunk_id = record.get("chunk_id") or record.get("chunk_type") or "chunk"
    return f"legal:{rel}#{chunk_id}"


def _read_legal_jsonl(path: Path) -> list[dict]:
    records = []
    with path.open("r", encoding="utf-8") as f:
        for line_number, line in enumerate(f, 1):
            ...
            if not text:
                continue
            source = record.get("source") or record.get("case_number") or path.parent.name
            records.append({
                "file": _legal_jsonl_manifest_key(path, record),
                "text": text,
                "source": source,
                "chunk": record.get("chunk_id") or f"{line_number:03d}",
                "metadata": record,
                "chunk_format": "legal_jsonl",
            })
    return records


def _read_chunk_records(chunk_file: Path) -> list[dict]:
    try:
        relative = chunk_file.relative_to(LEGAL_CHUNK_DIR)
    except ValueError as exc:
        raise ValueError(f"Unsupported chunk input for CGSIC indexing: {chunk_file}")
    if (
        len(relative.parts) != 2
        or not relative.parts[0].startswith("CGSIC_IMPORTANT_")
        or relative.parts[1] != "legal_chunks.jsonl"
    ):
        raise ValueError(f"Unsupported chunk input for CGSIC indexing: {chunk_file}")
    return _read_legal_jsonl(chunk_file)
...
def index_new_chunks():
    ...
    for chunk_file in new_chunks:
        try:
            chunks_data.extend(
                record
                for record in _read_chunk_records(chunk_file)
                if record["file"] not in indexed_files
            )
        except Exception as e:
    ...
    for i, (chunk_data, d_vector, s_embedding) in enumerate(
        ...
        payload = {
            ...
            "chunk_format": chunk_data.get("chunk_format", "generic_txt"),
        }
...
def show_status():
    ...
    # Chunk files
    logger.info(f"\n📁 Chunk Files:")
    legal_files = get_cgsic_chunk_files()
    legal_record_count = sum(
        len(_read_legal_jsonl(path))
        for path in legal_files
    )
    logger.info("  Generic txt indexing: disabled")
    logger.info(f"  CGSIC decision JSONL files: {len(legal_files)}")
    logger.info(f"  CGSIC chunk records on disk: {legal_record_count}")
    new_chunks = find_new_chunks()
    logger.info(f"  New files (not indexed): {len(new_chunks)}")
...
def main():
    ...
    parser.add_argument('--force', action='store_true',
                       help='Delete and recreate collection, then index all chunks')
    ...
    if args.recreate or args.force:
        if client.collection_exists(COLLECTION_NAME):
            ...
            manifest.clear()
        if args.recreate and not args.force:
            return
